...
void mlx_key(mlx_key_data_t key_game, void *param)
{
	...
	// ft_reles(key_game, game); // release the key
...
void	move_player(t_game *game, double move_x, double move_y)	// move the player
{
	...
	new_x = roundf(game->ply->player_x + move_x); // get the new x position
	new_y = roundf(game->ply->player_y + move_y); // get the new y position
